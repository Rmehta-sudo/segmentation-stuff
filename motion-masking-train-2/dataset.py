...
class Sample:
    ...
    mask: optional float32 array shaped (H, W) with values in {0,1}
    """
...
class MotionTeacherDataset:
    ...
    def __init__(
        ...
        mask_energy_percentile: float = 85.0,
    ):
        ...
        self.make_mask = bool(make_mask)
        self.mask_energy_percentile = float(mask_energy_percentile)
        if not (0.0 < self.mask_energy_percentile < 100.0):
            raise ValueError("mask_energy_percentile must be in (0,100)")
    ...
    def __getitem__(self, idx: int) -> Sample:
        ...
        if self.make_mask:
            # Derive an interpretable segmentation target from teacher energy.
            # We threshold energy by a per-frame percentile to be robust across videos.
            e = energy.astype(np.float32)
            if np.any(e > 0):
                thr = float(np.percentile(e[e > 0], self.mask_energy_percentile))
            else:
                thr = float(np.percentile(e, self.mask_energy_percentile))
            mask = (e >= thr).astype(np.float32)
