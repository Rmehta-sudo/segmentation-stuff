"""Teacher: illusion-aware motion-energy estimator.

This is the same script as `seg-2.py`, kept under a clear "teacher" name.

Importantly, it produces *training supervision tensors* per frame under:
  output/<video>_algo2_energy/frame_XXXXX/

Files per frame:
  - vx.npy (H,W) float32
  - vy.npy (H,W) float32
  - energy.npy (H,W) float32
  - speed.npy (H,W) float32 (optional)
  - vis.png (H,W,3) uint8 visualization

Run it from this directory so relative paths like `illusion-videos/...` resolve.
"""

# NOTE: We intentionally keep the implementation as a direct copy so it stays "frozen".
